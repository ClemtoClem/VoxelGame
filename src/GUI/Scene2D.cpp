...
Scene2D::Scene2D(int screenWidth, int screenHeight) 
    : _screenWidth(screenWidth), _screenHeight(screenHeight) {
    _projection = glm::ortho(0.0f, static_cast<float>(screenWidth), 0.0f, static_cast<float>(screenHeight));
}
...
void Scene2D::drawImage(GLuint textureID, const glm::vec2 &position, const glm::vec2 &size)
{
}
...
void Scene2D::setScreenSize(int width, int height)
{
    _screenWidth = width;
